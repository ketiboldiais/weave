...
~~~
let x = 5;
x = 10; --- environment-error: “x” is an immutable variable
var y = 3.14;
y = 5; --- “y” is mutable, so this is ok
~~~
...
1. The lexeme starts with an ASCII Latin character (lower- or upper-case), the underscore character (`“_”`), a dollar sign (`“$”`), or a single-quote (`“'”`).
2. The lexeme ends with an ASCII Latin character (lower- or upper-case), an ASCII digit (the Hindu-Arabic numerals `“0”` through `“9”`), an underscore character (`“_”`), a dollar sign (`“$”`), or a single-quote (`“'”`).
3. If the lexeme starts with a single-quote (`“'”`), the name must end with a single-quote (`“'”`).
4. Unless condition (3) is true, the name must not contain whitespace, operators, keywords, or control characters.
5. Unless condition (3) is true, the name must not be a reserved word.

Per condition (3), Twine permits variable names with whitespaces:

~~~
let x = 12 --- standard name
let 'New York City Pop.' = 8_468_000; --- name with whitespace
let '+' = 5; --- valid, but not a good idea
~~~

The single-quote in quoted identifiers are _part of the identifier_. Thus, the following code will err:


~~~
let 'cake and pie' = 15;
cake and pie = 15; --- syntax-error: invalid assignment target
~~~
...
~~~
let vector_A = [2.5, 4.1, 8.9, 13.7];
let a = vector_A[1] --- 2.5
~~~
...
Like other sequentials, we may access individual elements with an indexing expression:
...
## Classes
Twine supports classical object-oriented programming via classes.[^classical-oop]
